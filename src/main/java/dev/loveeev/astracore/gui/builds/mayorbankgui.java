...
public class mayorbankgui extends AdvancedMenu {

    Main plugin;
    public mayorbankgui(Player player,Main plugin) {
        super(player);
        this.plugin = plugin;
    }
    ...
    private Button getButtonInventory() {
        return new Button() {
            ...
            public ItemStack getItem() {
                ...
                lore.add("&fВы получаете каждый час по &#ffdaa4" + BuildsDatabase.moenybank(TownyAPI.getInstance().getTownName(getPlayer())) + " &fединиц вашей национальной валюты");

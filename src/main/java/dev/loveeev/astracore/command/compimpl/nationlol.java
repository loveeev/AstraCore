...
public class nationlol implements TabCompleter {
    @Nullable
    @Override
    public List<String> onTabComplete(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (nation != null) {
            return getPartialMatches(args[0], nation.getResidents().stream().map(resident -> resident.getName()).collect(Collectors.toList()));
        }
